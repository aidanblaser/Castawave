...
Y = [(cos(L * α / n )) * A + 1/6*A^4*k^3*cos(L*α/n) + A^2*k / 2 + A^4*k^3 / 2 for α in 1:n]
ϕ = [sqrt(GRAVITY*k)./k * A * exp.(Y[α]) * sin(X[α]) for α in 1:n]

xf, yf, ϕf, wl, ta = run(n, X, Y, ϕ, Δt, Float64(tf),Float64(L))
