...
#=
This is a script which rewrites Clamond's algorithm from MATLAB to Julia.
=#
...
function getIC(kd,kH2,N;tol=1e-14)
    ...
    Y = imag(zs);
    c = sqrt(9.81)*PP["cs"];
    F = c*imag(hilbert(imag(zs)))
    return X,Y,F,c    
end

function SSGW(kd,kH2,N;tol=1e-14)
...
# Revision: 2018-05-31.
#
# This code was translated into Julia by Aidan Blaser
#--------------------------------------------------------------------------
...
iter = 0;                                                                  # Iterations counter.                                                                     # Start clocking.

# Create global variables for quantities computed in while loop
Ys = [];
CYs = [];
C_hat = [];
Ys_hat = [];
...
while (err > tol)
  ...
  if d==Inf                                                                # Deep water.
     ...
     C_hat[1]  = 1/(sig*d);             # Operator C in Fourier space.
     S2_hat = (vk.*csch.((sig*d)*vk)).^2;  
     S2_hat[1] = 1/(sig*d)^2;           # Operator S^2 in Fourier space.
     Ys_hat  = fft(Ys);
  ...
  C_hat  =  vk.*coth.((sig.*d).*vk);
  C_hat[1] = 1/(sig*d);                   # Updated operator C in Fourier space.
...
# Post processing.
IH_hat = -im.*coth.(sig*d*vk);
IH_hat[1] = 0;                               # Inverse Hilbert transform.
Ys  = Ups .- mean(Ups);
Ys_hat  = fft(Ys);
...
# Output data.
PP = Dict("d"=>d,"k"=>k,"H"=>H,"ce"=>ce,"cs"=>cs,"B"=>B,"a"=>a,"b"=>b,
"intI"=>intI,"intV"=>intV,"intK"=>intK,"intSxx"=>intSxx,"intS"=>intS,"intF"=>intF,"cg"=>cg)
# Display results.
# subplot(211)
